const socket = io('http://172.16.15.25:3000');
    ...
    async function callAntrian(id) {
        try {
            const response = await fetch(`antrian/call/${id}`, { method: 'POST' });
    ...
    async function nextAntrian() {
        try {
            const response = await fetch('antrian/nextAntrian', { method: 'POST' });
    ...
    async function recallAntrian() {
        try {
            const response = await fetch('antrian/recall', { method: 'POST' });
    ...
    async function doneAntrian(id) {
        try {
            const response = await fetch(`antrian/done/${id}`, { method: 'POST' });
    ...
    async function doneList(id) {
        try {
            const response = await fetch(`antrian/doneList/${id}`, { method: 'POST' });
            if (!response.ok) throw new Error('Failed to mark queue as done');
    ...
    function playAudioSequence(queueNumber) {
        ...
        audioFiles.push('audio/loket14.mp3');
    ...
    async function loadData() {
        ...
        try {
            ...
            const list3 = await fetchData('antrian/prosesRacikan'); // Tambahkan bagian ini
            const antrianSekarang = await fetchData('antrian/antrianSekarang');
            ...
            <tr class="${item.sts_racikan === 'Y' && item.process_at === null ? 'alert-racikan' : ''}">
                ...
                <td>${item.no_reg}</td>
                <td>-</td>
                <td><button class="btn btn-outline-danger" onclick="updateProses('${item.id}')">Proses</button></td>
                <td><button class="btn btn-outline-success" onclick="doneList('${item.id}')">Done</button></td>
            </tr>
        `).join('');
            ...
            <tr class="${item.sts_racikan === 'Y' && item.process_at === null ? 'alert-racikan' : ''}">
                <td>${item.antrian}</td>
                <td>${item.no_reg}</td>
                <td><button class="btn btn-outline-primary" onclick="callAntrian('${item.id}')" ${item.antrian_aktif === 'Y' ? 'disabled' : ''}>Call</button></td>
                <td><button class="btn btn-outline-danger" onclick="updateProses('${item.id}')">Proses</button></td>
                <td><button class="btn btn-outline-success" onclick="doneList('${item.id}')">Done</button></td>
            </tr>
        `).join('');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        setInterval(loadData, 30000); // Memanggil loadData() setiap 5 detik
    });
